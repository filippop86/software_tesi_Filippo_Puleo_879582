...
#text = open("C://Users/filippo/Desktop/UNIMI/Tesi/esempio.txt","r").read()
#text = open("C://Users/filippo/Desktop/UNIMI/Tesi/agrotech/2015 and 2011.ris", "r", encoding="utf-8").read()
...
#text = "There is a compelling need for agricultural and forestry research to play a significant role in the future to meet the challenges of increased demand for food and fibre.\
'''This should be balanced against the need to deliver other vital services provided by land management, such as maintaining valued habitats. \
If this role is to be fulfilled, innovative research outputs need to be made accessible to end-users so that they can be applied in practice. \
Many EU and nationally funded research projects in the fields of agriculture and forestry provide excellent scientific results. \
However, outreach and interpretation of these results into ready-to-use farming and forestry formats is limited. \
Among the causes of limited dissemination of research outputs are the lack of a structured catalogue of research products and a limited diffusion in trade journals or in the form of practically oriented fact sheets devoted to final users such as farmers, advisors, and foresters. \
More broadly, three major developments have led to an increased need for effective uptake of research results into farming practice. \
Firstly, many threats facing agriculture, such as climate change, the vulnerability of food systems, food and water insecurity, soil degradation and the loss of biodiversity, represent large-scale challenges for the industry. \
Secondly, the policy agenda in Europe has changed with reorientation of the CAP (Common Agricultural Policy) and the increasing emphasis on the economic, environmental and social dimensions of sustainability. \
Thirdly, extension policy reforms of the 1990s in many EU countries have seen a move away from government-driven and funded Agricultural Knowledge and Innovation Systems (AKIS) towards pluralistic, market-driven, multi-actor systems, in which private actors have come to play a larger role (Rivera and Sulaiman, 2009). \
As a result, there are increasing demands on farmers to produce more with less, with more often explicitly including public goods and services, and they have to meet these demands in an environment where there is an increasing disconnection between research and farming."'''
...
def constr_satisf(enabl):
	...
	if (enabl == []):
		...
		for i in enabl:
			...
			sol = pattern_match.getSolution()

			x = remap(ebd,i)
			if ((enabl.index(i) == 0) or (j not in sol)):
...
def match_caus(l_sol):
	...
	for i in pat:   # per ogni pattern si verifica in quale frasi e' inserito ed i match eseguiti
		...
		print("PATTERN'S PROBABILITY TO HAVE COME FROM INDIPENDENT VARIABLES: {0:.2f}".format(p), '\n') 
